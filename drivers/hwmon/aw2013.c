...
static int led_const_on(struct aw2013_control_data *led)
{
	...
	//The value of led->grade_scale_fractions and led->grade_scale_numerator range from 1 to 5.
		if(led->min_grade == CONST_MIN_GRADE){
			write_reg(led,aw2013_regs.pwm0_max_lightness + led->outn, CONST_MIN_GRADE);
		}
		else{
			write_reg(led,aw2013_regs.pwm0_max_lightness + led->outn, 
				led->min_grade*BLINK_ONCE_MIN_GRADE_SCLACE + led->grade_scale_offset);
		...
		if(led->min_grade == CONST_MIN_GRADE){
			...
			write_reg(led,aw2013_regs.pwm0_max_lightness + led->outn, 
				led->min_grade*led->grade_scale_fractions/led->grade_scale_numerator+led->grade_scale_offset);
			LED_DEBUG("led_const_on,min_grade=%d . \n",led->min_grade*led->grade_scale_fractions/led->grade_scale_numerator+led->grade_scale_offset);
		}
	}
...
static int led_breath_once(struct aw2013_control_data *led)
{
	...
	if (led->min_grade == CONST_MIN_GRADE){	
		write_reg(led,aw2013_regs.pwm0_max_lightness, CONST_MIN_GRADE);
	}
	else{
		write_reg(led,aw2013_regs.pwm0_max_lightness, led->min_grade*BLINK_ONCE_MIN_GRADE_SCLACE+ led->grade_scale_offset);	
	}
	msleep((0x0d << led->Rise_Fall_time)*10 + 100);
	return 0;
...

{
	...
	ret = of_property_read_u32(client->dev.of_node, "grade_scale-fractions",
		&aw2013_data->grade_scale_fractions);
	if (ret < 0) {
		dev_err(&client->dev,"Failure reading grade_scale-fractions, rc =  %d\n", ret);
		return -EINVAL;
	}

	ret = of_property_read_u32(client->dev.of_node, "grade_scale-numerator",
		&aw2013_data->grade_scale_numerator);
	if (ret < 0) {
		dev_err(&client->dev,"Failure reading grade_scale-numerator, rc =  %d\n", ret);
		return -EINVAL;
	}
