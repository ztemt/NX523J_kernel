...
const int mms_key_lim[6] = {392,1176,
							326,978,
							417,1252};
...
int mms_dev_create(struct mms_ts_info *info)
{
	...
	if (alloc_chrdev_region(&info->mms_dev, 0, 1, MMS_DEVICE_NAME)) {
		TP_LOG_ERROR("failed to alloc_chrdev_region\n");
		ret = -ENOMEM;
		goto ERROR;
	}
	...
	if (cdev_add(&info->cdev, info->mms_dev, 1)) {
		TP_LOG_ERROR("failed to cdev_add\n");
		ret = -EIO;
		goto ERROR;
	}
...
/**
* Process open short data
*/
static int mms_proc_open_short_data(struct mms_ts_info *info,
	u8 data_type_size, u8 data_type_sign, u8 buf_addr_h, u8 buf_addr_l,
	u8 row_num, u8 col_num, u8 buf_col_num, u8 rotate, u8 key_num)
{
	int i_col = 0, i_row = 0;
	...
	int offset = buf_col_num * data_type_size;
	int data_size = data_type_size;
	int size = col_num * data_type_size;
	int has_key = 0;
	...
	for (i_row = 0; i_row < row_num; i_row++) {
		...
		for (i_col = 0; i_col < col_num; i_col++) {
			if (data_type_sign == 0) {//unsigned
				if (data_size == 1) {
					uValue = (u8)rbuf[i_col];
				} else if (data_size == 2) {
					uValue = (u16)(rbuf[data_size * i_col] | (rbuf[data_size * i_col + 1] << 8));
				} else if (data_size == 4) {
					uValue = (u32)(rbuf[data_size * i_col] | (rbuf[data_size * i_col + 1] << 8) \
						| (rbuf[data_size * i_col + 2] << 16) | (rbuf[data_size * i_col + 3] << 24));
				} else {
					dev_err(&info->client->dev, "%s [ERROR] data_size [%d]\n", __func__, data_size);
					goto ERROR;
				}
				...
				if (data_size == 1) {
					...
					sValue = (s32)(rbuf[data_size * i_col] | (rbuf[data_size * i_col + 1] << 8) \
						| (rbuf[data_size * i_col + 2] << 16) | (rbuf[data_size * i_col + 3] << 24));
				} else {
					dev_err(&info->client->dev, "%s [ERROR] data_size [%d]\n", __func__, data_size);
					goto ERROR;
			...
			dev_dbg(&info->client->dev, "value[%d][%d] = %d ", i_row, i_col, value);

			if ((value < MMS_OPEN_SHORT_LOW_LIM) || (value > MMS_OPEN_SHORT_HIGH_LIM)) {
				dev_err(&info->client->dev, "%s [ERROR] data[%d][%d] = %d\n",
					__func__, i_row, i_col, value);
				return 1;
			}
		}
	}
	...
	if (has_key) {
		buf_addr = (buf_addr_h << 8) | buf_addr_l | (offset * row_num);
		wbuf[0] = (buf_addr >> 8) & 0xFF;
		wbuf[1] = buf_addr & 0xFF;
		if (mms_i2c_read(info, wbuf, 2, rbuf, key_num * data_type_size)) {
			dev_err(&info->client->dev, "%s [ERROR] Read data buffer\n", __func__);
			goto ERROR;
		}

		for (i_col = 0; i_col < key_num; i_col++) {
			if (data_type_sign == 0) {//unsigned
				if (data_size == 1) {
					uValue = (u8)rbuf[i_col];
				} else if (data_size == 2) {
			...
			if ((value < mms_key_lim[i_col << 1]) || value > mms_key_lim[(i_col << 1) + 1]) {
				dev_err(&info->client->dev, "value[%d] = %d, min = %d, max = %d\n",
					i_col, value, mms_key_lim[i_col << 1], mms_key_lim[(i_col << 1) + 1]);
				return 1;
			}
		}
	}
...
{
	int busy_cnt = 50, wait_cnt = 50;
	u8 wbuf[8] = { 0 }, rbuf[512] = { 0 };
	u8 size = 0;
	u8 row_num = 0, col_num = 0;
	u8 buffer_col_num = 0;
	u8 rotate = 0;
	u8 key_num = 0;
	u8 data_type = 0, data_type_size = 0, data_type_sign = 0;
	u8 buf_addr_h = 0, buf_addr_l = 0;
	u8 actual_test_type = test_type;
	int ret = 0;

	TP_LOG_INFO("start - test_type[%d]\n", test_type);
	...
	switch (test_type) {
		...
		case MIP_TEST_TYPE_OPEN_SHORT:
			sprintf(info->print_buf, "\n=== Open Short Test ===\n\n");
			actual_test_type = MIP_TEST_TYPE_CM_DELTA;
			break;
		default:
			ret = 1;
			sprintf(info->print_buf, "\nERROR : Unknown test type\n\n");
			TP_LOG_ERROR("unknown test type\n");
			goto ERROR_EXIT;
			break;
	}
	...
	if (mms_i2c_write(info, wbuf, 3)) {
		ret = 1;
		TP_LOG_ERROR("failed to write test mode\n");
		goto ERROR_EXIT;
	}
	...
	if (wait_cnt <= 0) {
		ret = 1;
		TP_LOG_ERROR("wait timeout\n");
		goto ERROR_EXIT_TEST;
	}

	TP_LOG_DEBUG("set control mode\n");

	//set test type
	wbuf[0] = MIP_R0_TEST;
	wbuf[1] = MIP_R1_TEST_TYPE;
	wbuf[2] = actual_test_type;
	if (mms_i2c_write(info, wbuf, 3)) {
		ret = 1;
		TP_LOG_ERROR("failed to write test type\n");
		goto ERROR_EXIT_TEST;
	}

	TP_LOG_DEBUG("set test type\n");
	...
	if (mms_i2c_read(info, wbuf, 2, rbuf, 6)) {
		ret = 1;
		TP_LOG_ERROR("failed to read data format\n");
		goto ERROR_EXIT_TEST;
	}
	...
	TP_LOG_DEBUG("row_num[%d] col_num[%d] buffer_col_num[%d] rotate[%d] \
		key_num[%d]\n", row_num, col_num, buffer_col_num,
		rotate, key_num);
	TP_LOG_DEBUG("data_type[0x%02X] data_sign[%d] data_size[%d]\n", data_type,
		data_type_sign, data_type_size);
	...
	if (mms_i2c_read(info, wbuf, 2, rbuf, 2)) {
		ret = 1;
		TP_LOG_ERROR("failed to read buf addr\n");
		goto ERROR_EXIT_TEST;
	}

	buf_addr_l = rbuf[0];
	buf_addr_h = rbuf[1];
	TP_LOG_DEBUG("buf_addr[0x%02X 0x%02X]\n", buf_addr_h, buf_addr_l);

	//process data
	if (test_type == actual_test_type) {
		if (mms_proc_table_data(info, size, data_type_size, data_type_sign,
			...
			ret = 1;
			TP_LOG_ERROR("failed to mms_proc_table_data\n");
		}
	} else {
		if (mms_proc_open_short_data(info, data_type_size, data_type_sign, buf_addr_h,
			buf_addr_l, row_num, col_num, buffer_col_num, rotate, key_num)) {
			ret = 1;
			TP_LOG_ERROR("failed to mms_proc_open_short_data\n");
		}
	}
...
/**
* Sysfs run open short test
*/
static ssize_t mms_sys_open_short(struct device *dev, struct device_attribute *attr, char *buf)
{
	struct mms_ts_info *info = dev_get_drvdata(dev);
	int ret = 0;
	...
	ret = mms_run_test(info, MIP_TEST_TYPE_OPEN_SHORT);
	if (ret) {
		TP_LOG_ERROR("failed to mms_run_test\n");
		goto exit;
	}
...
static DEVICE_ATTR(power_off, 044, mms_sys_power_off, NULL);
...
static DEVICE_ATTR(open_short, 0444, mms_sys_open_short, NULL);
