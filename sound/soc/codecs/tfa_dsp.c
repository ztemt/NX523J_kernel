...
static int tfa98xx_startup(struct tfa98xx *tfa98xx)
{
	...
	*/
    ...
    if (tries == AREFS_TRIES) {
	   ...
	   pr_debug("tfa98xx_startup:  statusreg = 0x%04x, tries=%d\n", statusreg, tries);
    	}
...
int tfa98xx_is_configured(struct tfa98xx *tfa98xx)
{
	...
	/* check if SBSL bit is already set to configured DSP */
	configval = snd_soc_read(codec, TFA98XX_SYS_CTRL);

	pr_debug("SBSL %d\n", (configval & TFA98XX_SYS_CTRL_SBSL) != 0);

	return (configval & TFA98XX_SYS_CTRL_SBSL) != 0;
}
...
int tfa98xx_coldboot(struct tfa98xx *tfa98xx, int state)
{
	...
	while (state == !tfa98xx_is_coldboot(tfa98xx)) {
		...
		if (tries-- == 0) {
			pr_err("tfa98xx_coldboot Error:  coldboot (ACS) did not %s\n",
				  state ? "set" : "clear");
			return -EINVAL;
		}
	}
...
int tfa98xx_dsp_power_up(struct tfa98xx *tfa98xx)
{
	...
	if (tries == 0) {
		...
		pr_debug("tfa98xx_dsp_power_up:  tries=%d\n", (CFSTABLE_TRIES - tries));
	}
	...
	if (checkaddress != 0xffff) {
		...
		if (!ret) {
			if (!status) {
				...
				ret = -EBUSY;
				pr_err("tfa98xx check_ic_rom: DSP subsys not ready\n");
			}
		}
					 ...
					 const u8 *data)
{
	...
	if (ret)
		return ret;
	if(tfa98xx->rev != REV_TFA9890){  //TFA9890 can't support DRC feature
		ret = tfa98xx_dsp_support_drc(tfa98xx, &has_drc);
		if (ret)
			return ret;
...
int tfaRunSpeakerBoost(struct tfa98xx *tfa98xx, int force)
{
	...
	pr_debug("tfa98xx_SpeakerBoost:  force = %d\n", force);
	...
	if (force || tfa98xx_is_coldboot(tfa98xx)) {
		...
		pr_debug("tfa98xx_SpeakerBoost: coldstart%s\n", force ? " (forced)" : "");
		...
		if (!done) {
			...
			pr_err("tfa98xx_SpeakerBoost: Calibration not done!   CalibrationFailedRetry=%d\n", CalibrationFailedRetry);
			/*
			...
			pr_debug("tfa98xx_SpeakerBoost: Calibration done\n");
		}
		...
		if (!tfa98xx_is_calibrated(tfa98xx)){
			pr_err("tfa98xx_SpeakerBoost: Not calibrated yet!\n");
			done = 0;
		}
		else
			done = 1;		
    ...
    if (!done){
        pr_err("tfa98xx_SpeakerBoost: Not calibrated, power down devcie!\n");
...
int tfa98xx_dsp_start(struct tfa98xx *tfa98xx, int next_profile, int vstep)
{
	...
	if (forcecoldboot || tfa98xx_is_coldboot(tfa98xx)) {
		...
		if (tfaRunSpeakerBoost(tfa98xx, forcecoldboot)){
			pr_err("cold start up, tfa98xx SpeakerBoost error!\n");
		...
		if (next_profile != active_profile) {
			...
			if (!tfa98xx_is_configured(tfa98xx)) {    // added this for fixing xxxx issue after low temp test
...
int tfa98xx_dsp_stop(struct tfa98xx *tfa98xx)
{
	...
	status = snd_soc_read(codec, TFA98XX_STATUSREG);
	pr_err("tfa98xx_dsp_stop:  status register = 0x%04x\n", status);
	...
	{
		status = snd_soc_read(codec, TFA98XX_STATUSREG);
		pr_err("tfa98xx_dsp_stop:  mute device error, status reg = 0x%04x\n", status);
		return ret;
	}
