/*
 * Header file for:
 * Parade TrueTouch(TM) Standard Product (TTSP) touchscreen drivers.
 * For use with Parade Txx2xx and Txx3xx parts.
 *
...
#define CY_DRIVER_VERSION           "Rev3-2M-28-FF"
#define CY_DRIVER_DATE              "2015-11-24"


/*** ZTEMT Added by lixin, 2015/12/02 ***/
#define CY_PINCTRL_STATE_SLEEP "cyttsp3_pin_suspend"
#define CY_PINCTRL_STATE_DEFAULT "cyttsp3_pin_active"
/*** ZTEMT END  ***/
...
/* enable dev_dbg() */
#define DEBUG = y       //ztemt add for debug information                
...
#if 1 //ztemt modify for debug level
#define cyttsp_dbg(ts, l, f, a...) {\
	if (ts->bus_ops->tsdebug >= l) \
		dev_dbg(ts->dev, f, ## a);\
}
#else
#define cyttsp_dbg(ts, l, f, a...) dev_err(ts->dev, f, ## a)
#endif
#else
#define cyttsp_dbg(ts, l, f, a...)
...
#define ZTE_FACTORY_WRITE_BIN_BACK
#ifdef ZTE_FACTORY_WRITE_BIN_BACK
int pressure_file_write(char *file_path, const char *write_buf, unsigned int 
count);
int pressure_file_read(char *file_path, const char *read_buf, unsigned int 
count);
#endif
